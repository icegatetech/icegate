...
use icegate::{
    common::catalog::{CatalogBackend, CatalogBuilder, CatalogConfig},
    maintain::migrate::operations::{create_tables, upgrade_schemas},
};
use testcontainers::{
...
async fn test_migrate_create_tables() {
    ...
    let table_names: Vec<&str> = ops
        .iter()
        .map(|op| match op {
            icegate::maintain::migrate::operations::MigrationOperation::Create {
                table_name,
            } => table_name.as_str(),
            icegate::maintain::migrate::operations::MigrationOperation::Upgrade {
                ..
            } => {
