...
use icegate::common::catalog::{CatalogBuilder, CatalogConfig, CatalogBackend};
use icegate::maintain::migrate::operations::{create_tables, upgrade_schemas};
use testcontainers::{
...
async fn setup_containers() -> (
    ...
    println!("MinIO started on port {} (container: {}:9000 on network)", minio_port, minio_container_name);
    ...
    // Start Nessie catalog container (Iceberg REST compatible) on same network as MinIO
    ...
    // Note: with_wait_for must be called before with_network to stay on GenericImage
    let iceberg = GenericImage::new("quay.io/projectnessie/nessie", "0.105.7")
        .with_exposed_port(19120.tcp())
        .with_wait_for(WaitFor::http(
            testcontainers::core::wait::HttpWaitStrategy::new("/api/v2/config")
        ...
        .start()
        .await
        .expect("Failed to start Nessie container");

    let nessie_port = iceberg
        .get_host_port_ipv4(19120)
        .await
        .expect("Failed to get Nessie port");
    ...
    // Build catalog config - Nessie REST API is at /iceberg with prefix=main for branch
...
async fn test_migrate_create_tables() {
    ...
    let catalog = CatalogBuilder::from_config(&config)
        .await
    ...
    let table_names: Vec<&str> = ops
        .iter()
        .map(|op| match op {
            icegate::maintain::migrate::operations::MigrationOperation::Create { table_name } => {
                table_name.as_str()
            }
...
/// Test that `create_tables` with dry_run=true returns operations but doesn't create tables
#[tokio::test]
async fn test_migrate_create_tables_dry_run() {
    ...
    let ops = create_tables(&catalog, true)
        .await
...
async fn test_migrate_upgrade_schemas() {
    ...
    let create_ops = create_tables(&catalog, false)
        .await
        .expect("Failed to create tables");
    assert_eq!(create_ops.len(), 4);

    // Now call upgrade_schemas - should return 0 operations since schemas are current
    let upgrade_ops = upgrade_schemas(&catalog, false)
        .await
        .expect("Failed to upgrade schemas");
