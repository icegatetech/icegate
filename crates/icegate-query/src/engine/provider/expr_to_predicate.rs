// Forked from: https://github.com/apache/iceberg-rust/blob/main/crates/integrations/datafusion/src/physical_plan/expr_to_predicate.rs
// Reason: upstream module is `pub(crate)` and inaccessible from external crates.
...
pub fn convert_filters_to_predicate(filters: &[Expr]) -> Option<Predicate> {
    filters.iter().filter_map(convert_filter_to_predicate).reduce(Predicate::and)
}
...
fn convert_filter_to_predicate(expr: &Expr) -> Option<Predicate> {
    match to_iceberg_predicate(expr) {
        ...
        TransformedResult::NotTransformed => None,
    }
}
...
fn to_iceberg_predicate(expr: &Expr) -> TransformedResult {
    match expr {
        ...
        Expr::Literal(literal, _) => {
            scalar_value_to_datum(literal).map_or_else(|| TransformedResult::NotTransformed, TransformedResult::Literal)
        }
        Expr::InList(inlist) => {
            let mut datums = vec![];
        ...
        Expr::IsNull(expr) => {
            ...
            match p {
                TransformedResult::Column(r) => {
                    TransformedResult::Predicate(Predicate::Unary(UnaryExpression::new(PredicateOperator::IsNull, r)))
                }
                _ => TransformedResult::NotTransformed,
            }
        }
        Expr::IsNotNull(expr) => {
            ...
            match p {
                TransformedResult::Column(r) => {
                    TransformedResult::Predicate(Predicate::Unary(UnaryExpression::new(PredicateOperator::NotNull, r)))
                }
                _ => TransformedResult::NotTransformed,
            }
        }
        Expr::Cast(c) => {
            // DataFusion 54 replaced `Cast::data_type` with a full `FieldRef`.
        ...
        Expr::Like(Like {
            ...
            if pattern_str.ends_with('%') && !pattern_str[..pattern_str.len() - 1].contains(['%', '_']) {
                // Extract the prefix (remove trailing %)
                let prefix = pattern_str[..pattern_str.len() - 1].to_string();

                // Get the column reference
                let TransformedResult::Column(column) = to_iceberg_predicate(expr) else {
                    return TransformedResult::NotTransformed;
                };
...
const fn to_iceberg_operation(op: Operator) -> OpTransformedResult {
    match op {
...
fn to_iceberg_and_predicate(left: TransformedResult, right: TransformedResult) -> TransformedResult {
    match (left, right) {
...
fn to_iceberg_binary_predicate(
    ...
    let (r, d, op) = match (left, right) {
        (TransformedResult::NotTransformed, _) | (_, TransformedResult::NotTransformed) => {
            return TransformedResult::NotTransformed;
        }
        (TransformedResult::Column(r), TransformedResult::Literal(d)) => (r, d, op),
        (TransformedResult::Literal(d), TransformedResult::Column(r)) => (r, d, reverse_predicate_operator(op)),
        _ => return TransformedResult::NotTransformed,
    };
...
mod tests {
    ...
    fn create_test_schema() -> DFSchema {
        let arrow_schema = Schema::new(vec![
            ...
            Field::new("ts", DataType::Timestamp(TimeUnit::Second, None), true).with_metadata(HashMap::from([(
                PARQUET_FIELD_ID_META_KEY.to_string(),
                "3".to_string(),
            )])),
        ]);
    ...
    fn convert_to_iceberg_predicate(sql: &str) -> Option<Predicate> {
        ...
        let expr = SessionContext::new().parse_sql_expr(sql, &df_schema).unwrap();
    ...
    fn test_predicate_conversion_with_single_condition() {
        ...
        let predicate = convert_to_iceberg_predicate("foo != 1").unwrap();
        assert_eq!(predicate, Reference::new("foo").not_equal_to(Datum::long(1)));

        let predicate = convert_to_iceberg_predicate("foo > 1").unwrap();
        assert_eq!(predicate, Reference::new("foo").greater_than(Datum::long(1)));
        ...
        let predicate = convert_to_iceberg_predicate("foo <= 1").unwrap();
        assert_eq!(predicate, Reference::new("foo").less_than_or_equal_to(Datum::long(1)));
        ...
        assert_eq!(predicate, Reference::new("foo").is_in([Datum::long(5), Datum::long(6)]));
    ...
    fn test_predicate_conversion_with_complex_binary_expr() {
        ...
        let expected_predicate = Predicate::or(inner_predicate, Reference::new("foo").less_than(Datum::long(0)));
        assert_eq!(predicate, expected_predicate);
    }
    ...
    fn test_scalar_value_to_datum_timestamp() {
        ...
        let ts_micros = 1_672_876_800_000_000_i64; // 2023-01-05 00:00:00 UTC in microseconds
        let datum = super::scalar_value_to_datum(&ScalarValue::TimestampMicrosecond(Some(ts_micros), None));
        ...
        let ts_nanos = 1_672_876_800_000_000_500_i64; // 2023-01-05 00:00:00.000000500 UTC in nanoseconds
        let datum = super::scalar_value_to_datum(&ScalarValue::TimestampNanosecond(Some(ts_nanos), None));
        ...
        let ts_seconds = 1_672_876_800_i64; // 2023-01-05 00:00:00 UTC in seconds
        let datum = super::scalar_value_to_datum(&ScalarValue::TimestampSecond(Some(ts_seconds), None));
        assert_eq!(datum, None);

        let ts_millis = 1_672_876_800_000_i64; // 2023-01-05 00:00:00 UTC in milliseconds
        let datum = super::scalar_value_to_datum(&ScalarValue::TimestampMillisecond(Some(ts_millis), None));
        assert_eq!(datum, None);
    }
    ...
    fn test_predicate_conversion_with_like_starts_with() {
        ...
        let predicate = convert_to_iceberg_predicate(sql).unwrap();
        assert_eq!(predicate, Reference::new("bar").starts_with(Datum::string("test")));
    }
    ...
    fn test_predicate_conversion_with_not_like_starts_with() {
        ...
        let predicate = convert_to_iceberg_predicate(sql).unwrap();
        assert_eq!(predicate, Reference::new("bar").not_starts_with(Datum::string("test")));
    }
    ...
    fn test_predicate_conversion_with_like_empty_prefix() {
        let sql = "bar LIKE '%'";
        let predicate = convert_to_iceberg_predicate(sql).unwrap();
        assert_eq!(predicate, Reference::new("bar").starts_with(Datum::string("")));
    }
    ...
    fn test_predicate_conversion_with_like_unicode() {
        ...
        let sql = "bar LIKE '测试%'";
        let predicate = convert_to_iceberg_predicate(sql).unwrap();
        assert_eq!(predicate, Reference::new("bar").starts_with(Datum::string("测试")));
    }
}
