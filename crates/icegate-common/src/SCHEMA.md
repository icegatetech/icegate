...
- `logs` and `events` each split their single `attributes` map into `resource_attributes`, `scope_attributes`, and `log_attributes` (events rows are extracted from OTLP `LogRecord`s, so they carry `log_attributes` rather than a dedicated `event_attributes`).
...
- Breaking. `migrate upgrade` does not rewrite in place: drop `logs`, `spans`, `events`, and `metrics` through the catalog and re-run `migrate create`. Data in those four tables is lost. `operations` and `prices` are untouched.
