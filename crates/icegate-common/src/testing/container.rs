//! Object-storage testcontainer setup for integration tests and benchmarks.
//!
//! The container runs an S3-compatible object store. The public API here is
//! storage-agnostic (`S3TestContainer`) so the backend can change again without
//! renaming call sites.
...
/// Manifest digest (spike 2026-07-05): `sha256:fa19210ac4697c79d7ccca1ec9b0eb91aebacc6691991ffb14014bb3c67e6cc3`.
const RUSTFS_TAG: &str = "1.0.0-beta.8";
/// Root access key (used for both the container and downstream S3 SDK calls).
pub(crate) const STORAGE_ACCESS_KEY: &str = "rustfsadmin";
/// Root secret key (used for both the container and downstream S3 SDK calls).
pub(crate) const STORAGE_SECRET_KEY: &str = "rustfsadmin";

/// Manages an S3-compatible object-storage testcontainer for integration tests and benchmarks.
///
...
impl S3TestContainer {
    /// Start a new object-storage container and wait until it accepts TCP connections
    /// on the S3 port.
    ///
    /// # Errors
    ///
    ...
    pub async fn start() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // Wait until the S3 port accepts connections. Residual readiness races
        // (port open before S3 is fully serving) are absorbed by the retry loop
        // in `create_s3_bucket`, which every caller invokes next.
