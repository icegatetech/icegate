...
pub struct MergeInput {
    ...
    /// emitted in ascending `position` order. Sources MUST assign positions
    /// whose ascending order reproduces the desired stable order.
    ///
    /// `u128` is wide enough for the WAL source to pack
    /// `(segment_offset as u128) << 32 | (row_group_idx as u128)` and reproduce
    /// the old `(segment_offset, row_group_idx)` lexicographic order exactly
    /// without overflow.
    pub position: u128,
...
impl MergeInput {
    ...
    /// boundary range.
    #[must_use]
    pub const fn new(position: u128, bytes: u64, boundary_range: RowGroupBoundaryRange) -> Self {
        Self {
            position,
            bytes,
            boundary_range,
...
impl RowGroupsMergerObserver for NoopRowGroupsMergerObserver {
    fn on_input_opened(&self, _position: u128, _bytes: u64) {}

    fn on_input_closed(&self, _position: u128, _bytes: u64) {}
}
...
fn validate_unique_input_positions(read_plan: &[PlannedInputRead]) -> Result<()> {
    let mut seen_positions: HashMap<u128, usize> = HashMap::new();
    for input in read_plan {
...
mod tests {
    ...
    use super::{
        MergeInput, NoopRowGroupsMergerObserver, PlannedInputRead, RowGroupsMerger, RowGroupsMergerObserver,
    ...
    const fn position(segment_offset: u64, row_group_idx: usize) -> u128 {
        ((segment_offset as u128) << 32) | (row_group_idx as u128)
    }
    ...
    struct FakeMergeSource {
        batches: HashMap<u128, Vec<RecordBatch>>,
        open_delays: HashMap<u128, Duration>,
    ...
    struct FakeObserver {
        ...
        opened_at: Mutex<HashMap<u128, std::time::Instant>>,
    }
