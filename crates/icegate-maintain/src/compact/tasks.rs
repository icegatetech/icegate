...
//!
//! One runner per task code, each registered as the jobmanager [`TaskExecutor`] of that code.
//! A runner is the ONLY place that knows about the jobmanager: it opens the task's tracing
//! span, parses the serialized payload, drives the matching domain executor, and returns the
//! outcome the worker applies to the task. The domain modules
//! ([`crate::compact::data`], [`crate::compact::manifest`]) stay free of jobmanager
//! dependencies and are unit-testable without one.
//!
...
use jobmanager::{
    Error as JobError, TaskCode, TaskContext, TaskDefinition, TaskExecutor, TaskOutcome, TaskRef, TaskResult,
};
...
pub struct PlanTaskRunner {
    ...
    pub(super) compact_files_timeout: Duration,
    pub(super) compact_manifest_timeout: Duration,
...
impl PlanTaskRunner {
    ...
    async fn run(&self, ctx: &TaskContext) -> std::result::Result<TaskOutcome, JobError> {
        ...
        self.run_plan(ctx).instrument(span).await
    }
    ...
    /// `!Send`).
    ///
    /// Cancellation is checked before each potentially slow step (table load, data
    /// file enumeration) and before fanning out each REWRITE task, so a shutdown
    /// stops the PLAN promptly. A cancelled PLAN resolves to [`TaskOutcome::Cancelled`],
    /// so nothing it planned is persisted and the PLAN task is executed again later —
    /// planning is idempotent, and a partially fanned-out group set would otherwise be
    /// re-derived on top of the tasks already written.
    async fn run_plan(&self, ctx: &TaskContext) -> std::result::Result<TaskOutcome, JobError> {
        if ctx.is_cancelled() {
        ...
        let table = self
            ...
            .await
            .map_err(|e| JobError::Other(format!("failed to load table '{}': {e}", self.spec.table)))?;
        ...
        let stats = list_data_files_with_stats(&table, self.spec.descriptor)
            ...
            .await
            .map_err(|e| JobError::Other(format!("failed to enumerate data files: {e}")))?;
        ...
        // The REWRITE tasks fanned out this iteration; a `compact_manifest`
        // task depends on them so it runs only after data compaction settles.
        let mut rewrite_tasks: Vec<TaskRef> = Vec::new();
        ...
        for group in outcome.groups {
            // Stop fanning out promptly on shutdown. Nothing planned so far is
            // persisted, so the PLAN task re-runs from scratch next cycle.
            if ctx.is_cancelled() {
                return Ok(TaskOutcome::Cancelled);
            }
            ...
            let payload = serde_json::to_vec(&rewrite_input)
                .map_err(|e| JobError::Other(format!("failed to serialize rewrite input: {e}")))?;
            ...
            let files_task =
                TaskDefinition::new(TaskCode::new(FILES_TASK_CODE), self.compact_files_timeout).with_input(payload);
            rewrite_tasks.push(ctx.job().add_task(files_task)?);
        }
        ...
        let payload = serde_json::to_vec(&manifest_input)
            .map_err(|e| JobError::Other(format!("failed to serialize manifest compact input: {e}")))?;
        let manifest_task = TaskDefinition::new(TaskCode::new(MANIFEST_TASK_CODE), self.compact_manifest_timeout)
            .with_input(payload)
            .with_dependencies(rewrite_tasks);
        ctx.job().add_task(manifest_task)?;
...
#[async_trait]
impl TaskExecutor for PlanTaskRunner {
...
impl CompactFilesRunner {
    ...
    async fn run(&self, ctx: &TaskContext) -> Result<TaskOutcome, JobError> {
        ...
        self.run_compact(ctx).instrument(span).await
    }

    async fn run_compact(&self, ctx: &TaskContext) -> Result<TaskOutcome, JobError> {
        let input: RewriteInput = serde_json::from_slice(ctx.input())
            .map_err(|e| JobError::Other(format!("failed to parse rewrite input: {e}")))?;
        tracing::Span::current().record("partition", input.partition_key.as_str());
        link_planning_span(input.trace_context.as_deref());

        let outcome = self
            .executor
            .execute(&input, ctx.cancel_token())
            .await
            .map_err(|e| JobError::Other(format!("rewrite of table '{}' failed: {e}", self.table)))?;
...
#[async_trait]
impl TaskExecutor for CompactFilesRunner {
    async fn execute(&self, ctx: TaskContext) -> TaskResult {
        Ok(self.run(&ctx).await?)
    }
}
...
impl CompactManifestRunner {
    ...
    async fn run_compact(&self, ctx: &TaskContext) -> std::result::Result<TaskOutcome, JobError> {
        if ctx.is_cancelled() {
            return Ok(TaskOutcome::Cancelled);
        }
        let input: ManifestCompactInput = serde_json::from_slice(ctx.input())
            .map_err(|e| JobError::Other(format!("failed to parse manifest compact input: {e}")))?;
        link_planning_span(input.trace_context.as_deref());
        ...
        let outcome = self
            ...
            .execute(&input, ctx.cancel_token())
            .await
            .map_err(|e| JobError::Other(format!("manifest compaction of table '{}' failed: {e}", self.table)))?;
...
#[async_trait]
impl TaskExecutor for CompactManifestRunner {
    async fn execute(&self, ctx: TaskContext) -> TaskResult {
        Ok(self.run(&ctx).await?)
    }
}
