...
fn logs_batch(rows: &[(&str, i64)]) -> RecordBatch {
    ...
    // This fixture's merge/order assertions read only `service_name`/
    // `timestamp` (see `service_and_timestamp_rows`), so no attribute content
    // is needed at any OTLP level — all three per-level maps are empty rather
    // than the single empty `attributes` map this used before the schema
    // split.
